...
class Calendar extends React.Component {
  ...
  openEventDialog = (event) => {
    const type = event ? 'Edit Event' : 'Nuevo Evento';
    const newState = {
      event,
  ...
  addEvent = (result) => {
    ...
    this.setState({
      snackbarOpen: true,
      snackbarMessage: 'Evento Grabado'
    });
  }
  ...
  createRandomEvents = (number, startDate, endDate) => {
    const events = [];
    const eventNames = [
      'Visitar estación B. Reyes', 'Mandar mensaje a gerente de campo', 'Asistir a junta mensual', 'Planear app Billetigas', 'Ir a conferencia',
      'Reunión de gerentes generales', 'Llamar a Guadalupe', 'Planear reajuste de branding en estaciones', '', 'Visitar estaciones de Apodaca',
      'Hablar con gerente Miguel Alemán', 'Mandar Email a jefe', 'Junta operacional', 'Ir a convivio'];
